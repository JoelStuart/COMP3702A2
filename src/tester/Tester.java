...
public class Tester {
	...
	public boolean validState(ArmConfig state){
		if (hasSelfCollision(state)){
			return false;
		} if (state.hasGripper() && !hasValidGripperLengths(state)){
			return false;
		} if (!hasValidJointAngles(state)) {
			return false;
		} if (ps.getNumObstacles() > 0 && hasCollision(state, ps.getObstacles())){
			return false;
		} if (!fitsBounds(state)){
			return false;
		}
